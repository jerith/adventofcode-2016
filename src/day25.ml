...
let rec instr_to_str = function
  ...
  | OUT o -> spjoin ["out"; operand_to_str o]
...
module Comp = struct
  type t = computer
  let comptuple a = (a.ip, a.registers, a.instructions)
  let compare a b = compare (comptuple a) (comptuple b)
end

module CompSet = Set.Make(Comp)
...
let exec output computer =
  ...
  | TGL o -> output, {
      ...
      ip = computer.ip + 1}
  | OUT o -> get_value rs o :: output, {
      computer with
      ip = computer.ip + 1}
  | InvalidToggled _ -> output, {
      computer with
      ip = computer.ip + 1}
...
let exec_opt output computer =
  match find_opt computer [find_add; find_mul] with
  | Some (Add (ri, rd)) -> output, {
      computer with
      registers = add_reg computer ri rd;
      ip = computer.ip + 3}
  | Some (Mul (ri, rd, rm, o)) -> output, {
      computer with
      registers = mul_reg computer ri rd rm o;
      ip = computer.ip + 6}
  | None -> exec output computer

let check_output output =
  let rec ci' = function
    | [] -> true
    | [_] -> false
    | 0 :: 1 :: output -> ci' output
    | _ -> false
  in
  llen output > 0 && (ci' output || ci' (List.rev output))

let rec run states output computer =
  ifnoise (fun () -> print_computer computer);
  match computer.ip >= 0 && computer.ip < alen computer.instructions with
  | false -> noise_endline "Program halts."; false
  | true ->
    match exec_opt output computer with
    | output, state when CompSet.mem state states ->
      noise_endline @@ "Output: " ^ (map_to_str " " string_of_int output);
      check_output output
    | output, state -> run (CompSet.add state states) output state

let find_clock_input computer =
  let rec fci' i =
    let comp = {computer with registers = setreg i computer.registers 'a'} in
    match run (CompSet.singleton comp) [] comp with
    | false -> fci' (i+1)
    | true -> i
  in
  fci' 0
...
let main_1 input =
  ...
  find_clock_input computer |> string_of_int


let main_2 input = "All done!"
